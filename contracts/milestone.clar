...
(define-private (calculate-fee (amount uint) (user principal) (is-client bool))
  (let
    (
      (user-rep (default-to 
        { total-projects: u0, completed-projects: u0, total-earned: u0, total-spent: u0,
          average-rating: u0, total-ratings: u0, disputes-raised: u0, disputes-lost: u0,
          on-time-deliveries: u0, late-deliveries: u0, tier: "bronze" }
...
(define-private (calculate-tier (completed-projects uint) (average-rating uint))
...
(define-private (update-user-reputation (user principal) (updates { 
  ...
  (let
    (
      ...
      (new-completed-projects (+ (get completed-projects current-rep) (get completed-projects-delta updates)))
...
(define-private (add-user-project (user principal) (project-id uint))
  (let
    (
      (current-count (default-to { count: u0 } (map-get? user-project-count { user: user })))
      (index (get count current-count))
    )
    (map-set user-projects { user: user, index: index } { project-id: project-id })
...
(define-public (accept-proposal (project-id uint) (freelancer principal))
  (let
    (
...
(define-public (create-milestone
  ...
  (let
    ...
    (asserts! (or (is-eq (get status project) STATUS-ACTIVE) 
                  (is-eq (get status project) STATUS-OPEN)) ERR-INVALID-STATUS)
    ;; Payment amount must not exceed escrow balance
    (asserts! (<= payment-amount (get escrow-balance project)) ERR-INSUFFICIENT-FUNDS)
...
(define-public (submit-milestone (milestone-id uint) (deliverable-hash (buff 64)))
  (let
    (
    ...
    (asserts! (or (is-eq (get status milestone) MILESTONE-PENDING)
                  (is-eq (get status milestone) MILESTONE-REVISION)) ERR-INVALID-STATUS)
...
(define-public (approve-milestone (milestone-id uint))
  (let
    ...
    (asserts! (is-eq (get status milestone) MILESTONE-SUBMITTED) ERR-INVALID-STATUS)
    ...
    (let
      (
        ...
        (new-status (if all-milestones-completed STATUS-COMPLETED (get status project)))
      )
      (map-set projects
        { project-id: project-id }
        (merge project {
...
(define-public (request-revision (milestone-id uint) (feedback (string-utf8 512)))
  (let
    (
      (milestone (unwrap! (map-get? milestones { milestone-id: milestone-id }) ERR-MILESTONE-NOT-FOUND))
      (project-id (get project-id milestone))
...
(define-public (auto-approve-milestone (milestone-id uint))
  (let
    ...
    (asserts! (>= (- block-height submitted-at) AUTO-APPROVE-TIMEOUT) ERR-NOT-READY)
...
(define-public (raise-dispute 
  ...
  (let
    ...
    (asserts! (or (is-eq tx-sender (get client project))
                  (is-eq tx-sender freelancer)) ERR-NOT-AUTHORIZED)
    ;; Milestone must be submitted
    (asserts! (is-eq (get status milestone) MILESTONE-SUBMITTED) ERR-INVALID-STATUS)
...
(define-public (resolve-dispute 
  ...
  (let
    ...
    (try! (stx-transfer? dispute-fee loser (var-get platform-treasury)))
    ...
    (let
      (
        ...
        (new-completed-milestones (+ (get completed-milestones project) u1))
        (all-milestones-completed (is-eq new-completed-milestones (get milestones-count project)))
        (new-status (if all-milestones-completed STATUS-COMPLETED STATUS-ACTIVE))
      )
      (map-set projects
        { project-id: project-id }
        (merge project {
...
(define-public (cancel-project (project-id uint))
  (let
    ...
    (asserts! (not (is-eq (get status project) STATUS-DISPUTED)) ERR-INVALID-STATUS)
...
(define-public (rate-user 
  ...
  (let
    (
      (project (unwrap! (map-get? projects { project-id: project-id }) ERR-PROJECT-NOT-FOUND))
      (freelancer (unwrap! (get freelancer project) ERR-NOT-AUTHORIZED))
      (rated-user (if (is-eq tx-sender (get client project)) freelancer (get client project)))
...
(define-read-only (get-user-project (user principal) (index uint))
  (map-get? user-projects { user: user, index: index })
)
...
(define-read-only (get-platform-fee (amount uint) (user principal) (is-client bool))
...
(define-read-only (can-auto-approve (milestone-id uint))
  (match (map-get? milestones { milestone-id: milestone-id })
    milestone
      (match (get submitted-at milestone)
        submitted-at
          (ok (and 
